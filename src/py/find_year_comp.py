...
if len(np_cols) == 2:
    ...
    plt.title(f'New People Found: {previous_year} vs {current_year}')
    plt.xlabel('Week Number')
    plt.ylabel('New People Found')
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, 'find_year_comp_en.png'))
    plt.close()
...
if len(np_cols) == 2:
    ...
    plt.title(f'新しい見つけた人数: {previous_year}年と{current_year}年')
    plt.xlabel('週番号')
